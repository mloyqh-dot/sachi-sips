...
const DashboardPage: React.FC = () => {
  ...
  const metrics = useMemo(() => {
    ...
    return {
      totalRevenue,
      itemCount,
      averageTicket: filteredOrders.length > 0 ? totalRevenue / filteredOrders.length : 0,
  ...
  return (
    <div style={s.page}>
      <div style={s.shell}>
        ...
        {loading && <div style={s.alert}>Loading completed orders...</div>}
        {!loading && error && <div style={s.alert}>Unable to load dashboard: {error}</div>}
        {!loading && !error && orders.length === 0 && <div style={s.alert}>No completed orders found yet.</div>}
          ...
          <>
            <section style={s.metrics}>
              <div style={s.metric}>
                ...
                <span style={s.metricValue}>{formatCurrency(metrics.totalRevenue)}</span>
            ...
            <section style={s.sectionGrid}>
              <div style={s.panel}>
                ...
                <div style={s.rankedList}>
                  {metrics.topItems.map(([name, value]) => (
              ...
              <div style={s.panel}>
                ...
                <div style={s.rankedList}>
                  {metrics.staffTotals.map(([staff, value]) => (
