...
begin
  ...
  if p_subtotal is distinct from computed_subtotal then
    raise exception 'order subtotal does not match item totals';
  end if;
...
revoke all on function create_order(text, text, text, numeric, numeric, jsonb, text, text) from public;
revoke all on function create_order(text, text, text, numeric, numeric, jsonb, text, text) from anon;
revoke all on function create_order(text, text, text, numeric, numeric, jsonb, text, text) from authenticated;
grant execute on function create_order(text, text, text, numeric, numeric, jsonb, text, text) to service_role;
